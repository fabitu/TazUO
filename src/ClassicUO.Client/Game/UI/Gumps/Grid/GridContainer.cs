...
namespace ClassicUO.Game.UI.Gumps
{
    public class GridContainer : ResizableGump
    {
        ...
        private GumpPic openRegularGump, sortContents;
        private ResizableStaticPic quickDropBackpack;
        private GumpPicTiled backgroundTexture;
        private NiceButton setLootBag;
        ...
        public GridContainer(uint serial, ushort originalContainerGraphic, bool? useGridStyle = null) : base(GetWidth(), GetHeight(), GetWidth(2), GetHeight(1), serial, 0)
        {
            ...
            Point lastBackPackPos = ProfileManager.CurrentProfile.BackpackGridPosition;
            Point saveBackPackkSize = ProfileManager.CurrentProfile.BackpackGridSize;

            Point lastPos = GridSaveSystem.Instance.GetLastPosition(LocalSerial, isCorpse ? ProfileManager.CurrentProfile.DefaultCorpseGridSize : ProfileManager.CurrentProfile.DefaultGridSize);
            Point savedSize = GridSaveSystem.Instance.GetLastSize(LocalSerial, isCorpse ? ProfileManager.CurrentProfile.DefaultCorpseGridSize : ProfileManager.CurrentProfile.DefaultGridSize);

            if (IsPlayerBackpack)
            {
                lastPos = lastBackPackPos;
                savedSize = saveBackPackkSize;
            }
            else if (lastPos == lastBackPackPos)
            {
                lastPos = new Point(lastBackPackPos.X - 10 - savedSize.X, lastBackPackPos.Y);
            }
        ...
        private void BuildContainerDescription()
        {
        ...
        private void BuildSortContainers()
        {
        ...
        private void BuildQuickDropBackPack()
        {
        ...
        private string GetContainerName()
        {
            ...
            if (gridSlotManager != null)
            {
                ...
                containerName += $" ({gridSlotManager.ContainerContents.Count}) {container.Serial}";
            }
        ...
        public static void UpdateAllGridContainer()
        {
        ...
        public void HandleObjectMessage(Entity parent, string text, ushort hue)
        {
            if (parent != null)
            {
                ...
                if (item != null)
                {
                    UIManager.Add(new SimpleTimedTextGump(text, hue, TimeSpan.FromSeconds(2), 200) { X = item.ScreenCoordinateX, Y = item.ScreenCoordinateY });
                }
            }
        }
        ...
        public class GridSaveSystem
        {
            ...
            public bool SaveContainer(Item container, Dictionary<int, GridItem> gridSlots, int width, int height, int lastX = 100, int lastY = 100, bool? useOriginalContainer = false, bool autoSort = false, bool isPlayerBackPack = false, bool isPlayerBank = false)
            {
                try
                {
                    if (!enabled)
                    ...
                    XElement thisContainer = rootElement.Element("container_" + container.Serial.ToString());
                    if (thisContainer == null)
                    {
                        thisContainer = new XElement("container_" + container.Serial.ToString());
                    ...
                    if (isPlayerBackPack)
                        thisContainer.SetAttributeValue("isPlayerBackPack", isPlayerBackPack);
                    if (isPlayerBank)
                        thisContainer.SetAttributeValue("isPlayerBank", isPlayerBank);
            ...
            public Point GetLastSize(uint container, Point defaultGridSize)
            {
                ...
                {
                    lastSize = defaultGridSize;
                }

                return lastSize;
            }

            public Point GetLastPosition(uint container, Point defaultGridSize)
            {
                Point LastPos = new Point(lastX, lastY);
                ...
                {
                    var x = LastPos.X - 5 - defaultGridSize.X;
                    var y = LastPos.Y;
                    if (x <= 0)
                    {
                        x = LastPos.X;
                        y = y - 5 - defaultGridSize.Y;
                    }
                    if (y <= 0)
                        y = LastPos.Y;
